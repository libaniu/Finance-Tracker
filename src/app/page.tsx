...
export default function HomePage() {
  ...
  const [searchQuery, setSearchQuery] = useState(""); // BARU: State Pencarian
      ...
      <div className="fixed inset-0 w-full max-w-md bg-slate-50 h-dvh flex flex-col overflow-hidden shadow-2xl overscroll-none mx-auto">
        ...
        <nav className="flex-none bg-white border-t border-gray-100 py-3 px-15 flex justify-between items-center z-40 shadow-[0_-5px_10px_rgba(0,0,0,0.02)]">
          ...
          <Link href="/stats" className="flex flex-col items-center text-gray-400 hover:text-blue-600 transition-colors">
            <PieChart size={24} /> <span className="text-[10px] font-medium mt-1">Stats</span>
          </Link>
        </nav>
        ...
        {/* DELETE MODAL */}
        {deleteModal.show && (
          <div className="absolute inset-0 z-80flex items-center justify-center px-4">
        ...
        {/* TOAST */}
        <div className={`fixed top-4 left-1/2 transform -translate-x-1/2 z-90transition-all duration-300 ${toast.show ? "translate-y-0 opacity-100" : "-translate-y-10 opacity-0 pointer-events-none"}`}>
        ...
        {isDrawerOpen && (
          <div className="absolute inset-0 z-60flex flex-col justify-end">
            ...
            <div className="bg-white rounded-t-4xl p-6 relative z-10 animate-in slide-in-from-bottom duration-300">
              ...
              <form onSubmit={handleSubmit} className="space-y-4">
                ...
                <div>
                  ...
                  <input type="text" placeholder="e.g. Coffee" className="w-full bg-gray-50 p-4 rounded-xl font-medium text-gray-800 focus:outline-none focus:ring-2 focus:ring-blue-100" value={formData.title} onChange={(e) => setFormData({ ...formData, title: e.target.value })} required />
                </div>
                <div>
                ...
                <div>
                  ...
                  <div className="relative">
                    ...
                    <select value={formData.category} onChange={(e) => setFormData({...formData, category: e.target.value})} className="w-full bg-gray-50 p-4 pl-10 rounded-xl font-medium text-gray-800 focus:outline-none focus:ring-2 focus:ring-blue-100 appearance-none" required>
                      <option value="" disabled>Select Category</option>
                      {formData.type === 'expense' ? EXPENSE_CATEGORIES.map(cat => <option key={cat} value={cat}>{cat}</option>) : INCOME_CATEGORIES.map(cat => <option key={cat} value={cat}>{cat}</option>)}
                    </select>
                  </div>
                </div>
